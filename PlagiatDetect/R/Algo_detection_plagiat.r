...
#' Fonction distance de Levenshtein naïve en R
#'
...
lev = function(a, b){
  ...
  insertion <- lev(substr(a,1,nchar(a)-1), b) + 1

  deletion <- lev(a, substr(b,1,nchar(b)-1)) + 1

  substitution <- lev(substr(a,1,nchar(a)-1), substr(b,1,nchar( b)-1))

  if (substr(a, nchar(a), nchar(a)) != substr(b, nchar(b), nchar(b))){substitution = substitution + 1}
...
#' Fonction distance de Levenshtein optimisée en R
#'
